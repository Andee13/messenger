...
public class ClientListener extends Thread {
            ...
            switch (message.getStatus()) {
                ...
                case INVITE_USER:
                    if (isMessageNotFromThisLoggedClient(message)) {
                        ...
                        responseMessage = inviteClient(message);
                    }
                    break;
                case UNINVITE_USER:
                    if (isMessageNotFromThisLoggedClient(message)) {
                        ...
                        responseMessage = uninviteClient(message);
                    }
                case STOP_SERVER:
    ...
    private Message getClientInfo(Message message) {
        ...
        if (ClientProcessing.hasNotAccountBeenRegistered(server.getConfig(), message.getToId())) {
    ...
    private Message restartServer(Message message) {
        if ((isMessageNotFromThisLoggedClient(message)) && !message.getLogin().equals(server.getConfig().getProperty("serverLogin"))
                && !message.getPassword().equals(server.getConfig().getProperty("serverPassword"))) {
            return new Message(MessageStatus.DENIED).setText("Log in first");
        }
    ...
    private Message stopServer(@NotNull Message message) {
        ...
        if (!server.getConfig().getProperty("serverLogin").equals(message.getLogin())
                || !server.getConfig().getProperty("serverPassword").equals(message.getPassword())) {
    ...
    private Message clientBan(@NotNull Message message) {
        ...
        if (isMessageNotFromThisLoggedClient(message) && !(
                server.getConfig().getProperty("serverLogin").equals(message.getLogin())
                        && server.getConfig().getProperty("serverPassword").equals(message.getPassword())
                )) {
    ...
    private Message clientUnban(Message message) {
        ...
        Integer fromId = message.getFromId();
        if (ClientProcessing.hasNotAccountBeenRegistered(server.getConfig(), toId)) {
